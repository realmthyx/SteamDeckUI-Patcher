...
STEAM_PKG_PATH: Final[Path] = Path(sep.join((".steam", "steam", "package")))
STEAM_BETA_PATH: Final[Path] = Path(STEAM_PKG_PATH.joinpath("beta"))

def steamkill() -> int:
    "Kills the steam task"
    return os.system("killall steam")
...
def steam_patch() -> None:
    ...
    print("Script has applied the patch. Make sure you go to your Terminal and type steam -gamepadui or make a bash script.")
    time.sleep(3)
...
def main() -> None:
    ...
    print("Checking if Steam directory on your drive exists.")
